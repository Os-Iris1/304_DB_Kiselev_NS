...
$stmt = $pdo->prepare("SELECT * FROM working_hours WHERE schedule_id = ?");
$stmt->execute([$id]);
$schedule = $stmt->fetch();
...
if ($_POST) {
    ...
    if (in_array($day, range(1, 7)) && $start && $end && $end > $start) {
        $stmt = $pdo->prepare("
            ...
            WHERE schedule_id = ?
        ");
        $stmt->execute([$day, $start, $end, $notes, $id]);
...
<body>
    <h1>Редактировать смену — <?= htmlspecialchars($master['last_name'] . ' ' . $master['first_name']) ?></h1>
    <?php if ($message): ?>
    ...
    <form method="post">
        ...
        <p>
            <label>Примечания: <textarea name="notes" rows="3" style="width:100%"><?= htmlspecialchars($schedule['notes'] ?? '') ?></textarea></label>
        </p>
        <p>
